...
EXAMPLE_REF_FILE = _resolve_example_file("reference_addresses_london.csv")
...
def load_address_example(_example_key: str):
    ...
    if not (EXAMPLE_SEARCH_FILE.exists() and EXAMPLE_REF_FILE.exists()):
        expected_dirs = [
            str(Path.cwd() / "example_data"),
            str(Path(__file__).resolve().parents[1] / "example_data"),
        ]
        return (
            "Example files were not found in example_data/. Checked: "
            + " ; ".join(expected_dirs),
...
def main() -> None:
    """
    Console-script entry point for running the GUI after `pip install fuzzy_address_matcher`.
    """
    block = build_app()
    if COGNITO_AUTH:
        block.queue().launch(
