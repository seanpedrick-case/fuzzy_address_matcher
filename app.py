...
with block:
    ...
    with gr.Tab("Match addresses"):
        ...
        with gr.Accordion(
            "I have multiple addresses in a CSV/XLSX/Parquet file", open=True
        ):
            ...
            in_colnames = gr.Dropdown(
                ...
                choices=[],
                multiselect=True,
                label="Select columns that make up the address. Make sure postcode is at the end",
            )
            ...
            use_postcode_blocker = gr.Checkbox(
                label="Use postcode blocker (untick to use street-only blocking)",
                value=True,
            )
    ...
    with gr.Tab(label="Advanced options"):
        with gr.Accordion(label="AWS data access", open=False):
            aws_password_box = gr.Textbox(
                label="Password for AWS data access (ask the Data team if you don't have this)"
            )
            with gr.Row():
                in_aws_file = gr.Dropdown(
                    label="Choose keyword file to load from AWS (only valid for API Gateway app)",
                    choices=["None", "Lambeth address data example file"],
                )
                load_aws_data_button = gr.Button(
                    value="Load keyword data from AWS", variant="secondary"
                )

            aws_log_box = gr.Textbox(label="AWS data load status")

    ### Loading AWS data ###
    load_aws_data_button.click(
        fn=load_data_from_aws,
        inputs=[in_aws_file, aws_password_box],
        outputs=[in_ref, aws_log_box],
    )
    ...
    submit_feedback_btn.click(
        lambda *args: feedback_callback.flag(list(args)),
    ...
    estimated_time_taken_number.change(
        lambda *args: usage_callback.flag(list(args)),
        [session_hash_textbox, in_file, estimated_time_taken_number],
        None,
        preprocess=False,
    ).then(
        fn=upload_file_to_s3,
